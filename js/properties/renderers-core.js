...
import { prctx } from './context.js?v=1.24.7';
import { updateContainerHeaderLayout, updateDataObjectHeaderLayout, updateNoteIconLayout, updateSimpleNodeLayout } from '../canvas.js?v=1.24.7';
import { SVG as COMPONENT_SVG, contrastTextColor, extractLinkDomain, getStencilSvgDataUri, resizeDataObjectToFit } from '../components.js?v=1.24.7';
import { startImageAddFlow } from '../image-component.js?v=1.24.7';
import { recolorCellIcon } from './color-schema.js?v=1.24.7';
import { DASH_BY_LINE_STYLE, BRACKET_PATHS, BRACKET_LABEL_X } from '../persistence/diagram-schema.js?v=1.24.7';
import { convertFromIcon, convertToContainer, convertToIcon, convertToNode, convertPlaceholderToNode } from './convert.js?v=1.24.7';
import { renderFieldEditor } from './field-editor.js?v=1.24.7';
import { finishStandardProps } from './render-core.js?v=1.24.7';
import { addAutoSizeBtn, addChipInput, addCloneBtn, addColor, addDeleteBtn, addIconPicker, addNumber, addNumberPair, addOrderButtons, addRaciPicker, addSegmented, addSelect, addText, addTextarea, section, wireMarkdownShortcuts } from './widgets.js?v=1.24.7';
...
export function renderPlaceholderProps(cell) {
  ...
  // The way OUT. A placeholder is temporary by definition, and deleting it to draw a real node would take every
  // connector with it - so resolving one that was already wired in meant re-drawing the wiring by hand. Convert
  // swaps the cell in place: links, container parent, position, size and selection all survive, one undo step.
  finishStandardProps(cell, {
    sizeMode: 'pair',
    autoSize: true,
    convert: [{ label: 'Convert to Node', onClick: () => convertPlaceholderToNode(cell) }],
  });
}
