...
import { csvCell, escHtml, sanitizeFilenamePart, toMarkdownTable } from './util.js?v=1.24.7';
import { getActiveTabName, getActiveTabType } from './tabs.js?v=1.24.7';
// The flow tab's own glyph (a zero-import leaf), wrapped below as the flow table's nav-button icon.
import { diagramTypeIconMarkup } from './tabs/diagram-types.js?v=1.24.7';
import { startBatch, endBatch, setLocked, undo } from './history.js?v=1.24.7';
import { SF_FIELD_TYPES } from './properties.js?v=1.24.7';
import { keyImpliesRequired } from './field-model.js?v=1.24.7';
import { buildModal, showToast, showError } from './feedback.js?v=1.24.7';
import { buildObjectSchemaCsv } from './data-export.js?v=1.24.7';
import { triggerDownload } from './persistence.js?v=1.24.7';
import { ganttRowLayout, ganttDependencies, ganttTimelineFor, applyGanttGeometry, resequenceGanttOrders } from './gantt-layout.js?v=1.24.7';
import { durationDays, addDaysISO } from './gantt-scale.js?v=1.24.7';
...
} from './table-view/builders.js?v=1.24.7';
...
import { announce } from './a11y.js?v=1.24.7';
...
import { addGanttTask, deleteGanttBar, reorderGanttBar, openDepEditor, initGanttPlan } from './table-view/gantt-plan.js?v=1.24.7';
...
import { initFlowTable, renderFlowTable, resetFlowTableFilter, exportFlowCsvFromGraph, FLOW_TABLE_TITLE, SEC_CHEVRON } from './table-view/flow-table.js?v=1.24.7';
import { buildModelRelationships, REL_COLUMNS } from './table-view/builders.js?v=1.24.7';
...
export function init(modules) {
  ...
  if (graph) {
    ...
    // ...and the Gantt plan's fields: an "Edit Plan" session changed a bar's Start, the user dragged that bar in the
    // Diagram view, and Save then wrote the table's start with the drag's end - a span nobody entered, possibly with
    // end before start, and no warning (audit 2026-09-23). Same guard for the plan table.
    graph.on('change:startDate change:endDate change:taskLabel change:order change:groupId change:progress change:assignee', onDiagramSchemaEdit);
...
function exportRowsCsv(rows, cols = COLUMNS, suffix = 'mapping') {
  const esc = v => {
    ...
    return csvCell(s === '—' ? '' : s);   // RFC-4180 quoting + the spreadsheet formula guard
  };
...
export function exportMappingCsv() {
  if (!graph) return;
  exportRowsCsv(buildData(graph, { showUnmapped: _showUnmapped }).rows);
}

// Save → Export to CSV entry (Flow, 2026-09-24): the flow table's file built from the graph, table open or not.
export function exportFlowCsv() { exportFlowCsvFromGraph(); }
