...
import { GANTT_HEADER_H, GANTT_BAR_DY, orderToY } from './gantt-layout.js?v=1.24.7';
import { localISODate } from './gantt-scale.js?v=1.24.7';
import { sanitizeCssColor } from './util.js?v=1.24.7';
import { SEQ_ROLE_ACCENT, BPMN_EVENT_STYLE, BPMN_GATEWAY_GLYPH } from './persistence/diagram-schema.js?v=1.24.7';
...
import { node, SVG, GENERIC_SHAPES } from './components/stencil-kit.js?v=1.24.7';
import { FLOW_ELEMENTS } from './shapes/flow.js?v=1.24.7';
...
// node() / container() / SVG (glyph map) / GENERIC_SHAPES moved to ./components/stencil-kit.js (S9);
...
export function createElementFromComponent(component, position = { x: 100, y: 100 }) {
  ...
  switch (type) {
    ...
    case 'sf.GanttTimeline': {
      ...
      const endDate = localISODate(endD);   // LOCAL day - toISOString() is the UTC day, one early in UTC+ zones
