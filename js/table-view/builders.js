...
import { ganttRowLayout, ganttDependencies } from '../gantt-layout.js?v=1.24.1';
import { durationDays } from '../gantt-scale.js?v=1.24.1';
import { FLOW_ELEMENTS } from '../shapes/flow.js?v=1.24.1';
...
export function sortRows(rows, { sortKey, sortDir } = {}) {
  ...
  return rows
    .map((r, i) => [r, i])
    .sort((a, b) => {
      // NATURAL order: numbers inside the text compare as numbers, so Length "20" sorts before "100" and "Field2"
      // before "Field10" (a plain string compare put "100" first; audit 2026-09-23). Case-insensitive as before.
      const av = String(a[0][sortKey] ?? '');
      const bv = String(b[0][sortKey] ?? '');
      const c = av.localeCompare(bv, 'en', { numeric: true, sensitivity: 'base' });
      if (c) return c * dir;
