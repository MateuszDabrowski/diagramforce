...
import { showToast, showError } from '../feedback.js?v=1.24.7';
import {
...
import { selectOnly } from '../selection.js?v=1.24.7';
...
function exportFlowCsv() {
  downloadFlowCsv(flowCsvText(_lastGrids), filterActive());
}

/** Save > Export as CSV on a flow tab (2026-09-24): the same file, built from the graph so it works with the table
 *  closed. It carries every row in the default order - no table sort or filter applies - the contract
 *  exportMappingCsv keeps for Data Mapping. The in-table button stays export-what-you-see. */
export function exportFlowCsvFromGraph() {
  if (!graph) return;
  const grids = buildFlowSections(graph).sections.map(sec => ({ id: sec.id, title: sec.title, rows: sec.rows, allCols: sec.columns, _matches: null }));
  downloadFlowCsv(flowCsvText(grids), false);
}

function downloadFlowCsv(text, filtered) {
  const blob = new Blob([text], { type: 'text/csv;charset=utf-8;' });
  triggerDownload(URL.createObjectURL(blob), `df_${sanitizeFilenamePart(getActiveTabName(), 'tab')}_flow${filtered ? '_filtered' : ''}.csv`);
}

function flowCsvText(grids) {
  const esc = csvCell;   // RFC-4180 quoting + the spreadsheet formula guard (util.js)
