...
import { createOverlay } from './overlay-layer.js?v=1.21.4';
...
export function highlightLooseConnectors() {
  ...
  overlay.deactivate();                 // clear any previous highlight (resets _ids via onTeardown)
  _ids = loose.map((x) => x.id);
  if (!_ids.length) return 0;
  overlay.activate();                   // first draw + subscribe (redraw on move, auto-clear on reset)
  const bb = looseBBox();
